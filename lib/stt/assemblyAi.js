...
const transcribeAssemblyAi = async (logger, socket) => {
    ...
    try {
      if (!isBinary) {
        ...
        if (obj.type === "start") {
          ...
          logger.info(
            { token, sampleRateHz },
            "transcribeAssemblyAi: got token",
          );
          ...
          assemblyAiSocket
            .on("message", (buffer) => {
              ...
              if (
                !data.message_type ||
                ["SessionBegins"].includes(data.message_type)
              )
                return;
        ...
        if (socket.assemblyAiSocket) {
          ...
          if (socket.audioBuffer.length > 4) {
            const base64Data = Buffer.concat(socket.audioBuffer).toString(
              "base64",
            );
            socket.audioBuffer = [];
  ...
  socket.on("end", (err) => {
    logger.error(
      { err },
      "transcribeAssemblyAi: socket closed from Cognigy Voice Gateway",
    );
