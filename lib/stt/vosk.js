...
const {
  ...
  RecognitionConfig,
  RecognitionSpec,
} = require("../../stubs/vosk/vosk_stt_pb");
const grpc = require("@grpc/grpc-js");
const assert = require("assert");

const createVoskClient = async () => {
  const client = new SttServiceClient(
    process.env.VOSK_URL,
...
const transcribeVosk = async (logger, socket) => {
      ...
      if (!isBinary) {
        ...
        if (obj.type === "start") {
          ...
          stream.on("data", function (response) {
            ...
            logger.info(
              { data },
              "sending transcription to Cognigy Voice Gateway",
            );
            const obj = {
              type: "transcription",
              is_final,
  ...
  socket.on("end", (err) => {
    logger.error(
      { err },
      "transcribeVosk: socket closed from Cognigy Voice Gateway",
    );
    closeVoskStream(socket);
  });
};
