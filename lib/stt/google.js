...
const transcribeGoogle = async (logger, socket) => {
  assert.ok(
    process.env.GCP_JSON_KEY_FILE,
    "GCP_JSON_KEY_FILE env var is required for google speech",
  );
  ...
  socket.on("message", function (data, isBinary) {
    try {
      if (!isBinary) {
        ...
        logger.info(
          { obj },
          "received JSON message from Cognigy Voice Gateway",
        );
        if (obj.type === "start") {
          assert.ok(
            !socket.recognizeStream,
          ...
          socket.recognizeStream = client
            ...
            .on("data", (data) => {
              logger.info(
                ...
                "received data from recognize stream",
              );
  ...
  socket.on("end", function (err) {
    logger.error(
      { err },
      "transcribeGoogle: socket closed from Cognigy Voice Gateway",
    );
  });
};
