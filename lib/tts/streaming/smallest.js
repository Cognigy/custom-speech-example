...
const ttsStreamingSmallest = async (logger, socket, url) => {
  ...
  const params = url.includes('?')
    ? Object.fromEntries(url.split('?')[1].split('&').map((p) => p.split('=')))
    : {};

  const voiceId = params.voice || 'meher';
  const sampleRate = parseInt(params.sampleRate || '24000', 10);
  const model = params.model || 'lightning_v3.1_pro';
  ...
  const openSmallestSocket = () => {
    if (smallestSocket && smallestSocket.readyState === WebSocket.OPEN) return;
