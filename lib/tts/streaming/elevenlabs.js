...
const ttsStreamingElevenlabs = async (logger, socket, url) => {
  ...
  ellSocket
    ...
    .on("end", (err) => {
      logger.info(
        { err },
        "ttsStreamingElevenlabs: socket closed from elevenlabs",
      );
    });
  socket.on("message", async (data, isBinary) => {
    ...
    socket.on("error", (err) => {
      logger.error(
        { err },
        "ttsStreamingElevenlabs: Cognigy Voice Gateway error",
      );
    });
    socket.on("close", (data) => {
      logger.info(
        { data },
        "ttsStreamingElevenlabs: Cognigy Voice Gateway close",
      );
      terminateSocket(socket);
    });
    socket.on("end", (err) => {
      logger.error(
        { err },
        "ttsStreamingElevenlabs: socket closed from Cognigy Voice Gateway",
      );
      terminateSocket(socket);
    });
  });
