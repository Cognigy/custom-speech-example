...
routes.post('/', async(req, res) => {
  ...
  try {
    ...
    if (String(type).toLowerCase() === 'ssml') {
    ...
    const response = await fetch(`${RIME_URL}/v1/rime-tts`, {
      method: 'POST',
      headers: {
      ...
      signal: AbortSignal.timeout(Number(process.env.RIME_TIMEOUT_MS) || 30000)
    });
    if (!response.ok) {
      const detail = await response.text();
      logger.error({statusCode: response.status, detail}, 'synthAudio: Error synthesizing speech using rime');
    ...
    logger.error({err: err.message}, 'synthAudio: Error synthesizing speech using rime');
    res.status(timedOut ? 504 : 500).json({
      error: timedOut ? 'rime request timed out' : err.message
    });
  }
});
